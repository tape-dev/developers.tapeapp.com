...
export const MyComponent = ({children, color}) => 
{
  ...
  const query = `{"operationName":"reportActiveUserPresence","variables":{"input":{}},"query":"mutation reportActiveUserPresence($input: ReportActiveUserPresenceInput!) {\n  defaultAlias: reportActiveUserPresence(input: $input) {\n    ...VoidNoNesting\n    __typename\n  }\n}\n\nfragment VoidNoNesting on Void {\n  _void\n  __typename\n}\n"}`;
   useEffect(() => {
    ...
    fetch('https://tapeapp.com/graphql', requestOptions)
      .then(res => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
