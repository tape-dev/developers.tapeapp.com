...
`single_date` field-values can only hold a single date, while `range_date` fields can hold both single dates and date ranges consisting of a start and end date.
