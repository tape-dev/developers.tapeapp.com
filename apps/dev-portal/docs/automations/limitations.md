...
- A single record-collection action (e.g. `collect_app_records` / `collect_app_view_records`) returns at most **1000 records**. A configured `limit` above 1000 — or with `limit_enabled: false`, or non-positive — is stored and read back **unchanged** (there is no write-time or `validate` warning); the cap is applied only when the run executes, where the collection is silently truncated to exactly 1000, visible only in the run log. (This is separate from the per-run action cap above.)
