...
For security reasons it could be necessary to whitelist the IP addresses of the Tape automation servers when making network connections to your infrastructure or service provider. Make sure to whitelist the IP addresses listed below to avoid network or firewall issues.

When dispatching remote HTTP operations inside a Tape automation, the worker node will use one of the following IP addresses:
